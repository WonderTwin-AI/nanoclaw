...
describe('matrix skill package', () => {
  ...
  it('has all files declared in adds', () => {
    const addFile = path.join(skillDir, 'add', 'src', 'channels', 'matrix.ts');
    ...
    const testFile = path.join(skillDir, 'add', 'src', 'channels', 'matrix.test.ts');
  ...
  it('has all files declared in modifies', () => {
    ...
    expect(indexContent).toContain('MatrixChannel');
    expect(indexContent).toContain('MATRIX_HOMESERVER');
    expect(indexContent).toContain('MATRIX_ONLY');
    expect(indexContent).toContain('findChannel');
    expect(indexContent).toContain('channels: Channel[]');

    const configContent = fs.readFileSync(configFile, 'utf-8');
    expect(configContent).toContain('MATRIX_HOMESERVER');
    expect(configContent).toContain('MATRIX_USER_ID');
    expect(configContent).toContain('MATRIX_ONLY');
    expect(configContent).toContain('MATRIX_ENCRYPTION');
  });
  ...
  it('modified index.ts includes Matrix channel creation', () => {
    const content = fs.readFileSync(
      path.join(skillDir, 'modify', 'src', 'index.ts'),
      'utf-8',
    );
    ...
    expect(content).toContain('channels.push(matrix)');

    // Conditional channel creation
    expect(content).toContain('if (!MATRIX_ONLY)');
    expect(content).toContain('if (MATRIX_HOMESERVER && MATRIX_USER_ID)');
